...
def basic_operations():
	...
	# define the source
	'''
NB10010236,Y,50,문화,11,JTBC,"10시 뉴스, 2시간짜리 드라마·시트콤 … 지상파에 없는 새로운 볼거리가 온다"
NB10010573,R,10,정치,11,JTBC,"한미 FTA 비준, 부작용 점검해 대비책 마련해야"
NB10010589,N,20,경제,11,JTBC,"이순신대교, 세계가 놀랄 신기록 보유"
NB10010617,Y,20,경제,11,JTBC,"광저우모터쇼에 나온 중국 신차 ""성장세 이어갈 것"""
	'''
	table = t_env.from_elements(
		elements=[
			('NB10010236', '{"section": "문화", "source": "JTBC", "title": "10시 뉴스, 2시간짜리 드라마·시트콤 … 지상파에 없는 새로운 볼거리가 온다", "reporter": {"depart": "국제외교안부", "name": "김필규"}}'),
			('NB10010573', '{"section": "정치", "source": JTBC, "title": "한미 FTA 비준, 부작용 점검해 대비책 마련해야", "reporter": {"depart": "정책부", "name": "임소라"}}'),
			('NB10010589', '{"section": "경제", "source": JTBC, "title": 이순신대교, 세계가 놀랄 신기록 보유", "reporter": {"depart": "중앙일보", "name": "배노필"}'),
			('NB10010617', '{"section": "경제", "source": JTBC, "title": "광저우모터쇼에 나온 중국 신차 ""성장세 이어갈 것", "reporter": {"depart": "중앙일보", "name": "이현상"}')
		],
		schema=['id', 'data'])

	right_table = t_env.from_elements(elements=[('NB10010236', 'Y'), ('NB10010573', 'N'), ('NB10010589', 'Y'), ('NB10010617', 'Y')],
									  schema=['id', 'used_yn'])

	table = table.add_columns(
		col('data').json_value('$.section', DataTypes.STRING()).alias('section'),
		col('data').json_value('$.source', DataTypes.STRING()).alias('source'),
		col('data').json_value('$.title', DataTypes.STRING()).alias('title'),
		col('data').json_value('$.reporter.depart', DataTypes.STRING()).alias('reporter.depart'),
		col('data').json_value('$.reporter.name', DataTypes.STRING()).alias('reporter.name')) \
